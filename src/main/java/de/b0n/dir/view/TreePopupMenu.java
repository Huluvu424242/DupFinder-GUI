...
public class TreePopupMenu extends JPopupMenu implements ActionListener {
	...
	public TreePopupMenu(final TreeEntryNode node, final JTree tree) {
		this.node = node;
		this.tree = tree;
	...
	private void removeNodeFromTree() {
		if (getNode().isLeaf()) {
			TreeEntryNode dupParent = (TreeEntryNode) getNode().getParent();
			...
			if (getNode().getChildCount() > 1) {
				...
				Enumeration<TreeEntryNode> children = getNode().children();
				TreeEntryNode newDupParent = children.nextElement();
				// create list of old and new children nodes
				List<TreeEntryNode> newChildren = new ArrayList<TreeEntryNode>();
				while (children.hasMoreElements()) {
					TreeEntryNode newChildrenNode = children.nextElement();
				...
				// insert new children
				for (TreeEntryNode newChildrenNode : newChildren) {
					newDupParent.add(newChildrenNode);
				}
				// expand path
				getTree().expandPath(new TreePath(((TreeEntryNode) newDupParent).getPath()));
			} else {
